# -------------------------------------------------------------------------
# Title:    Data preparation - FPJT OpenSesame local
# Author:   Carla Czilczer
# Date:     25.08.2026
# R ver.:   4.5.2
#
# Purpose:
# Prepare OpenSesame local experiment output (.csv files) so that:
...
# Place all OpenSesame subject-*.csv files in that "data" folder.
...
# Build data folder path: <script_dir>/data; if unknown, use cwd/data
if (!is.null(script_path)) {
...
if (!dir.exists(data_dir)) {
  stop("Required 'data' folder not found next to this script:\n  ",
       ...
       "\nPlease ensure that the 'data' folder is present next to the script.")
}

setwd(data_dir)
message("Working directory set to data folder: ", normalizePath(getwd()))

# -------- Workflow: read multiple OpenSesame CSV files and merge ---------
...
# Read one CSV file safely. Empty or unreadable files are skipped.
...
# Combine files even if they differ slightly in their columns.
...
if (length(missing_item_cols) > 0L) {
  stop("Required FPJT item columns are missing from the OpenSesame CSV: ",
       paste(missing_item_cols, collapse = ", "),
       "\nThese variables must be initialized before the automatic logger is prepared. ",
       "The randomized item order cannot be reconstructed from this CSV.")
}
