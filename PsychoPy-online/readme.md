...
3.  Click the green **Run** button to test locally (debugging only; not recommended for online data collection).
4.  To run online, use PsychoPy’s **Pavlovia** workflow (e.g., “Pavlovia → Sync” / “Export HTML”), and follow the standard PsychoPy/Pavlovia procedure to create/sync a Pavlovia project.
5.  **Distribute** the generated study link to your participants. They run the task directly in their web browser.
6.  To **export data**:
    - **Local runs:** output files are saved into the `data` folder (see [OUTPUT](#OUTPUT)).  
    - **Online runs (Pavlovia):** download results from the Pavlovia project dashboard (Results/Data export).
...
1. Open the experiment file `FPJT_online.psyexp`
...
### 5. Ensure the updated files are available for online export
For local runs, PsychoPy reads the spreadsheets directly from disk. For online runs, you must re-export / re-sync so the updated resources are uploaded.
1. Save the experiment in PsychoPy.
2. Re-export for online use (PsychoJS) and re-sync to Pavlovia (or your host).

### 6. Export / Sync for online use
In PsychoPy Builder, use the Pavlovia workflow (e.g., “Pavlovia → Sync”). This creates/updates the online version of the study on your host.  

---
> **⚠️ Important:** You **MUST NOT** change the names of the folders or files, as this will cause the experiment to crash (especially online, where resources are matched by path). Additionally, do not change any variable names; the experiment logic depends on these specific identifiers, and renaming them requires updating the underlying code. Do not move files after decompressing the repository. Any deviation from the original file structure or naming can lead to a crash.

---------------------------------------
## TECHNICAL DETAILS
The decompressed repository includes the following files and subfolders:
* `FPJT_online.psyexp`: The main PsychoPy experiment file; needed to change the experiment settings.
...
To try out the experiment after changing settings or adding a new language, click the green **Run** button in PsychoPy. (This mode is suitable for debugging, but not for online data collection!)

Once you have finished your configuration, you must export/sync the experiment for online use:
1.  **Save** the experiment in PsychoPy.
2.  **Export HTML / Sync** via the Pavlovia workflow.
3.  **Distribute** the Pavlovia study link.
...
For online runs (e.g., Pavlovia), data is stored on the server/host and can be downloaded from the project’s results/data export interface.
...
PsychoPy / PsychoJS version updates might require adjustments in the experiment file.  
