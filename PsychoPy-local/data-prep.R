# -------------------------------------------------------------------------
# Title:    Data preparation - FPJT PsychoPy local
# Author:   Carla Czilczer
# Date:     04.06.2026
# R ver.:   4.5.2
#
# Purpose:
# Prepare PsychoPy local experiment output (.csv files) so that:
...
# Place all PsychoPy local or Pavlovia .csv files in that "data" folder.
...
# PsychoPy local may store these settings under the shorter names
# response_mode and feedback. Use them if the selected_* columns are absent.
if (!"selected_response_mode" %in% names(df) && "response_mode" %in% names(df)) {
  df$selected_response_mode <- df$response_mode
}

if (!"selected_feedback" %in% names(df) && "feedback" %in% names(df)) {
  df$selected_feedback <- df$feedback
}
...
# Online output usually stores test responses in response.*.
# PsychoPy local may also store them in trials_loop.response.*.
...
# Test block
key_col <- if ("response.keys" %in% names(df) &&
               any(!is.na(df$response.keys) & nzchar(trimws(as.character(df$response.keys))))) {
  "response.keys"
} else if ("trials_loop.response.keys" %in% names(df)) {
  "trials_loop.response.keys"
} else {
  stop("No FPJT test response column found. Expected 'response.keys' or 'trials_loop.response.keys'.")
}

corr_col <- if ("response.corr" %in% names(df)) "response.corr" else "trials_loop.response.corr"
rt_col   <- if ("response.rt"   %in% names(df)) "response.rt"   else "trials_loop.response.rt"

test_rows <- !is.na(df[[key_col]]) & nzchar(trimws(as.character(df[[key_col]])))
...
if (any(test_rows)) {
  ...
  test_tbl$fpjt_correct <- as_int(get_col(df[test_rows, , drop = FALSE], corr_col))
  test_tbl$fpjt_rt <- as_num(get_col(df[test_rows, , drop = FALSE], rt_col))
  test_tbl$trial_response <- as.character(get_col(df[test_rows, , drop = FALSE], key_col))
}

if (nrow(test_tbl) == 0L) {
  stop("No FPJT test response rows found.")
}
